...
Protocol (length-prefixed — safe for atoms containing any bytes):
  1. Serialise: 4-byte count + (4-byte len + utf8 bytes) per string.
  2. Allgather byte counts, Allgatherv payloads.
  3. Deserialise length-prefixed records.
...
function mpi_allgatherv_strings(local_strs::Vector{String}) :: Vector{String}
    ...
    # Serialise: length-prefixed records (safe for any atom content)
    buf = IOBuffer()
    write(buf, Int32(length(local_strs)))
    for s in local_strs
        b = codeunits(s)
        write(buf, Int32(length(b)))
        write(buf, b)
    end
    local_bytes = take!(buf)
    local_len   = Int32(length(local_bytes))

    # Gather byte-buffer lengths from all ranks
    all_lens = MPI.Allgather(local_len, _MPI_COMM[])
    ...
    # Deserialise length-prefixed records from all ranks
    result = String[]
    io = IOBuffer(recv_buf)
    for _ in all_lens
        n_strs = read(io, Int32)
        for _ in 1:n_strs
            slen = read(io, Int32)
            push!(result, String(read(io, slen)))
        end
    end
    result
end
